...
<style>
  ...
  .stMain [data-testid="stVerticalBlockBorderWrapper"] { background: #fff; border-radius: 10px; }
  ...
  .rd-progress { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:10px; margin: 0 0 20px; padding:12px 14px; background:#fff; border:1px solid #e2e8f0; border-radius:12px; }
  .rd-progress-step { display:flex; align-items:center; gap:6px; min-width:0; color:#64748b; font-size:.82rem; white-space:normal; padding:8px; border-radius:6px; }
  @media (max-width:760px) { .rd-progress { grid-template-columns:repeat(2,minmax(0,1fr)); } }
  .rd-progress-step.selected { outline:2px solid #94a3b8; outline-offset:2px; }
  .rd-progress-step::before { content:''; display:block; width:9px; height:9px; border-radius:50%; background:#cbd5e1; flex:0 0 auto; }
  .rd-progress-step.done { color:#166534; font-weight:650; }
  .rd-progress-step.done::before { background:#16a34a; }
  .rd-progress-step.current { color:#1d4ed8; font-weight:700; }
  .rd-progress-step.current::before { background:#2563eb; box-shadow:0 0 0 3px #dbeafe; }
  .rd-progress-step.alert { color:#9a3412; font-weight:650; }
  .rd-progress-step.alert::before { background:#f97316; }
  .rd-progress-step.failed { color:#b91c1c; font-weight:700; }
  .rd-progress-step.failed::before { background:#dc2626; }
...
def show_run_details(run_id: str):
    ...
    with st.expander("查看运行日志", expanded=is_active or bool(failed)):
        show_run_logs(state)
